...
app.get("/api/server/status", legacyAuth, (req, res) => {
  // Kita gunakan rumus bash script bawaan kamu dari file system monitor
  const script = `
    ram=$(free -m | awk 'NR==2{printf "%.2f", $3*100/$2 }')
    cpu=$(top -bn1 | grep "Cpu(s)" | sed "s/.*, *\\([0-9.]*\\)%* id.*/\\1/" | awk '{print 100 - $1}')
    up=$(uptime -p)
    
    # Format output menjadi JSON
    echo '{"cpu": "'"$cpu"'", "ram": "'"$ram"'", "uptime": "'"$up"'"}'
  `;

  exec(script, (err, stdout) => {
    if (err) {
      return res.status(500).json({ success: false, message: "Gagal membaca resource VPS" });
    }
    ...
    try {
      ...
      res.json({
        success: true,
        status: "ON",
        cpu_usage: parseFloat(sysData.cpu),
        ram_usage: parseFloat(sysData.ram),
        uptime: sysData.uptime
      });
    } catch (e) {
      res.status(500).json({ success: false, message: "Parsing output error", raw: stdout });
    }
  });
});
