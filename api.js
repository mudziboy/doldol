...
function saveQuota(protocol, user, quotaGb) {
  ...
  try {
    // Memastikan folder ada sebelum menulis (opsional namun disarankan)
...
function runBinary(res, cmd, args, quotaInfo = null) {
  ...
  child.on('close', code => {
    ...
    try {
      ...
      // INTEGRASI SAVE QUOTA JIKA PROSES BERHASIL
      if (quotaInfo && quotaInfo.user && quotaInfo.quota) {
...
function createXray(proto, req, res) {
  ...
  // Menambahkan quotaInfo agar tersimpan di file .quota
  runBinary(res, "bash", ["-c", args.join(" ")], { proto, user, quota });
}
...
function createZiVPN(res, password, days, ipLimit, quota) {
  runBinary(res, "/usr/bin/apicreate-zivpn", [
    ...
    String(days),
    String(ipLimit)
  ], { proto: "zivpn", user: password, quota: quota });
}

function renewZiVPN(res, password, days, quota) {
...
app.get("/renewzivpn", legacyAuth, (req, res) => {
  const { password, exp, quota } = req.query;
  if (!password || !exp) {
    return res.status(400).json({ status: "error", message: "Invalid parameter" });
  }
  renewZiVPN(res, password, exp, quota);
});
...
app.post("/api/user/renew", headerAuth, (req, res) => {
  const { password, days, quota_gb } = req.body;
  if (!password || !days) {
    return res.status(400).json({ success: false, error: "Invalid payload" });
  }
  renewZiVPN(res, password, days, quota_gb);
});
