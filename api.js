...
app.get("/createzivpn", legacyAuth, (req, res) => {
  const { password, expired, iplimit, quota } = req.query;

  console.log(`[API] Create ZiVPN request`);

  // VALIDASI PARAMETER
  if (!password || !expired || !iplimit) {
    return res.status(400).json({
      status: "error",
      message: "Missing parameter (password, expired, iplimit)"
    });
  }

  // NORMALISASI
  const days = String(expired);
  const ip = String(iplimit);
  const qt = quota ? String(quota) : null;

  // ARGUMENT SESUAI SCRIPT
  const args = [
    "apicreate-zivpn",
    password,
    days,
  ...
  runBinary(
    ...
    {
      proto: "zivpn",
      user: password, // fallback (karena tidak ada username)
      quota: qt,
      iplimit: ip
    }
  );
