...
// Tambahkan handler khusus untuk createzivpn
app.get("/createzivpn", (req, res) => {
  const { user, password, expired, iplimit, quota } = req.query;
  
  // Log untuk debugging
  console.log(`[API] Received Create ZiVPN: ${user}`);

  // Jalankan script bash apicreate dengan type 'zivpn'
  const child = spawn("bash", ["/usr/bin/apicreate", "zivpn", user, password, expired, iplimit]);
  ...
  child.on("close", (code) => {
    if (code === 0) {
      // Simpan kuota jika ada
      if (quota && quota !== "0") {
        saveQuota("zivpn", user, quota);
      }
      try {
        res.json(JSON.parse(output));
      } catch (e) {
        res.status(500).json({ status: "error", message: "Invalid JSON from script", details: output });
      }
    } else {
      res.status(500).json({ status: "error", message: "Script failed", details: output });
    }
  });
});
