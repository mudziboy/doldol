...
app.get("/createzivpn", legacyAuth, (req, res) => {
  const { user, exp, iplimit, quota } = req.query;

  console.log(`[API] Create ZiVPN request`, req.query);

  // VALIDASI PARAMETER
  if (!user || !exp || !iplimit) {
    return res.status(400).json({
      status: "error",
      message: "Missing parameter (user, exp, iplimit)"
    });
  }

  // NORMALISASI
  const password = String(user); // zivpn pakai password sebagai username
  const days = String(exp);
