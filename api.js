...
/* ======================================================
   UTIL: SAVE IP LIMIT (NEW INTEGRATION)
====================================================== */
...
function runBinary(res, cmd, args, quotaInfo = null) {
  ...
  child.on('close', code => {
    ...
    try {
      ...
      // INTEGRASI SAVE QUOTA & IP LIMIT JIKA PROSES BERHASIL
...
/* ======================================================
   ZIVPN CORE LOGIC
====================================================== */
function createZiVPN(res, password, days, ipLimit, quota) {
  runBinary(res, "/usr/bin/apicreate-zivpn", [
    password,
    String(days),
    String(ipLimit)
  ], { proto: "zivpn", user: password, quota: quota, iplimit: ipLimit });
}

function renewZiVPN(res, password, days, quota, iplimit) {
...
function trialZiVPN(res, durationMinutes, ipLimit) {
  ...
  runBinary(res, "/usr/bin/apicreate-zivpn", [
    ...
    String(days),
    String(ipLimit)
  ], { proto: "zivpn", user: password, iplimit: ipLimit });
}
...
app.get("/createzivpn", legacyAuth, (req, res) => {
  const { password, exp, iplimit, quota } = req.query;
  if (!password || !exp || !iplimit) {
    return res.status(400).json({ status: "error", message: "Invalid parameter" });
  }
  createZiVPN(res, password, exp, iplimit, quota);
});

app.get("/renewzivpn", legacyAuth, (req, res) => {
  const { password, exp, quota, iplimit } = req.query;
  if (!password || !exp) {
    return res.status(400).json({ status: "error", message: "Invalid parameter" });
  }
  renewZiVPN(res, password, exp, quota, iplimit);
});
...
app.post("/api/user/create", headerAuth, (req, res) => {
  ...
  createZiVPN(res, password, days, ip_limit, quota_gb);
});

app.post("/api/user/renew", headerAuth, (req, res) => {
  const { password, days, quota_gb, ip_limit } = req.body;
  if (!password || !days) {
    return res.status(400).json({ success: false, error: "Invalid payload" });
  }
  renewZiVPN(res, password, days, quota_gb, ip_limit);
});
...
/* ======================================================
   GET REMAINING QUOTA (ON-DEMAND)
====================================================== */
app.get("/api/user/quota", headerAuth, (req, res) => {
  ...
  try {
    if (fs.existsSync(path)) {
      ...
      res.json({ success: true, quota: quotaValue });
    } else {
      // Jika file .quota tidak ada, anggap Unlimited atau belum terpakai
      res.json({ success: true, quota: "Unlimited" });
    }
  } catch (err) {
