...
/* ======================================================
   XRAY & SSH ENDPOINTS (FIXED & MERGED)
====================================================== */

// Endpoint untuk Xray (Vmess, Vless, Trojan, Shadowsocks)
app.post("/api/user/create/xray", headerAuth, (req, res) => {
  ...
  // Cek apakah menggunakan binary spesifik (apicreate-vmess) atau umum (apicreate)
  const binSpesifik = `/usr/bin/apicreate-${protocol}`;
  const binUmum = "/usr/bin/apicreate";
  
  const bin = fs.existsSync(binSpesifik) ? binSpesifik : binUmum;
  const args = bin === binUmum ? [protocol, user, days, "0", ip_limit] : [user, days, ip_limit];
  ...
  child.on("close", () => {
    try {
      ...
      console.error("Xray Create Error:", output);
      res.status(500).json({ success: false, message: "Gagal memproses output", output });
    }
  });
});
